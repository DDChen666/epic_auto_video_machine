...
async function main() {
  ...
  try {
    ...
    if (!existsSync(envPath)) {
      console.log(
        '⚠️  .env.local not found. Please create it with your database configuration.'
      )
      console.log(
        '   You can copy from .env.example and update the DATABASE_URL'
      )
      process.exit(1)
    }
    ...
    if (
      ...
      envContent.includes('your-database-url')
    ) {
    ...
    console.log(
      '   • Visit http://localhost:3000/api/health to check system health'
    )
  } catch (error) {
    ...
    console.log(
      '   • For Neon Postgres, ensure you have the correct connection string'
    )
