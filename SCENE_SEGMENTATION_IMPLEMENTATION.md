...
class SceneSegmentationService {
  ...
  segmentText(text: string, options?: SegmentationOptions): Promise<SegmentationResult>
  editScenes(scenes: SceneData[], operation: SceneEditOperation): Promise<SceneData[]>
  validateScenes(scenes: SceneData[], config?: SegmentationConfig): ValidationResult
  
  // Text processing
  private normalizeChineseText(text: string): string
  private ruleBasedSegmentation(text: string, config: SegmentationConfig): SceneData[]
  private llmAssistedSegmentation(text: string, config: SegmentationConfig): Promise<SceneData[]>
}
```
