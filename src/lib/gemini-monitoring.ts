...
export class GeminiMonitoring {
  ...
  getPerformanceIndicators(userId?: string): {
    ...
    const rateLimitErrors = recentMetrics.filter(m => m.error === GeminiErrorType.RATE_LIMIT).length
  ...
  checkUserHealth(userId: string): {
    ...
    if (indicators.averageLatency > 5000) {
      ...
      recommendations.push('Consider using regional endpoints or reducing request complexity')
    }
    
    if (indicators.rateLimitHits > 5) {
      issues.push(`Frequent rate limiting: ${indicators.rateLimitHits} hits in last minute`)
   ...
   */
  private formatPrometheusMetrics(allMetrics: Record<string, GeminiMetrics>): string {
    let output = ''
    
    // Total requests
    output += '# HELP gemini_requests_total Total number of Gemini API requests\n'
