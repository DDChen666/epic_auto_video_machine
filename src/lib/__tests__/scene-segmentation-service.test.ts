...
第二段落的內容，也包含足夠的文字來形成另一個場景。這應該是另一個場景的開始，同樣需要滿足最小長度要求才能成為獨立場景。這是第二段的更多內容。`
      ...
      expect(result.scenes.some(scene => scene.text.includes('第一段落'))).toBe(
        true
      )
    })
    ...
    it('should split by sentences using Chinese punctuation', async () => {
      const text =
        '第一句話包含足夠的內容來測試句子切分功能，這句話很長。第二句話也包含足夠的內容來測試分段！第三句話同樣有足夠的長度來測試？第四句話繼續測試分段功能；第五句話完成測試並結束。'
      const result = await service.segmentText(text, {
      ...
      const scenes: SceneData[] = [
        {
          id: 'scene_0',
          index: 0,
          text: '這是一個符合長度要求的場景內容，包含足夠的文字來通過驗證測試。',
        },
        {
          id: 'scene_1',
          index: 1,
          text: '另一個有效的場景內容，也符合最小和最大長度的要求。',
        },
      ]
