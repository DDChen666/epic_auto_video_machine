 ...
 */
export const GET = withMetrics(withRateLimit(apiRateLimit, withApiHandler(async (request: NextRequest) => {
  const { searchParams } = new URL(request.url)
  
  // Parse query parameters with defaults
  const page = parseInt(searchParams.get('page') || '1', 10)
  const limit = Math.min(parseInt(searchParams.get('limit') || '20', 10), 100)
  const status = searchParams.get('status') as 'draft' | 'ready' | 'processing' | 'completed' | 'failed' | null

  // Validate parameters
  if (isNaN(page) || page < 1) {
    return errorResponse(ERROR_CODES.INVALID_INPUT, 'Invalid page parameter', HTTP_STATUS.BAD_REQUEST)
  }
  if (isNaN(limit) || limit < 1) {
    return errorResponse(ERROR_CODES.INVALID_INPUT, 'Invalid limit parameter', HTTP_STATUS.BAD_REQUEST)
  }
  if (status && !['draft', 'ready', 'processing', 'completed', 'failed'].includes(status)) {
    return errorResponse(ERROR_CODES.INVALID_INPUT, 'Invalid status parameter', HTTP_STATUS.BAD_REQUEST)
  }
 ...
 */
export const POST = withMetrics(withRateLimit(apiRateLimit, withApiHandler(async (request: NextRequest) => {
  let body
  try {
